# The Tools Used in `ngBase`

## Introduction

`ngBase` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `ngBase` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`ngBase` uses git to manage its codebase. While in theory you don't have
to use Git once you download `ngBase`, this project makes the assumption
...
While `ngBase` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `ngBase` you have to tell NPM to install the
...
to get started with `ngBase`. But for those curious or looking to go a
...
Grunt is the engine behind `ngBase`. It's the magic that makes it move.
